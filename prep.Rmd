...
The anonymized data-sets are deposited in DANS EASY. You can request permission to download the data-sets [here](link).

Download the data-files `ego_public.R`, `alter_public.R`, and `data_public.R` and put them in the `./data/` folder. But first, make a `./data/` folder: 
...
fsave(ego, file="ego_public.R")
fsave(alter, file="alter_public.R")
```
...
- `ego_public.R`: anonymized data of the additional ego-survey
- `alter_public.R`: anonymized data of the additional alter-survey
...
```{r, eval=FALSE}
ego <- fload("./data/ego_public.R")
alter <- fload("./data/alter_public.R")
dat <- fload("./data/data_public.R")
```
...
In case you don't wish to replicate the data-set, you may obtain it by downloading: `r xfun::embed_file("./data shared/output.R")`
...
#  print(i)
  ...
  #LR
  df$LR.percepton[which(df$respnr==i)] <- 
...
df[,17:length(df)] <- lapply(names(df)[-c(1:16)], function(x) as.numeric(df[[x]]))

{
df$LR.accuracy <- 1-abs(df$LR.percepton-df$LR.truth)/11
...
for (i in unique(df$respnr)) {
  ...
  #LR
  df$LR.percepton[which(df$respnr==i)] <- as.numeric(stringr::str_extract(alterdata$P1X, pattern = "\\(?[0-9,.]+\\)?"))
...
#names(df)
#but first, convert positions to numeric
df[,16:length(df)] <- lapply(names(df)[-c(1:15)], function(x) as.numeric(df[[x]]))
...
```{r,eval=F}
#add a column indicating whether it is the ego or alter response/perspective
df.ego$response = "ego"
df.alter$response = "alter"
...
```{r, echo=FALSE}
df <- fload("./data/processed/20230419output.R")
